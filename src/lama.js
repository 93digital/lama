...
(function ($) {
   ...
   */
  var dateInput = document.querySelectorAll('.lama-date');
  dateInput.forEach(date => {
    date.addEventListener('input', function (e) {
      var _form = $(date).closest('form')[0];
      submitForm(_form);
    });
  });
  ...
  for (let index = 0; index < lamaSearch.length; index++) {
    const input = lamaSearch[index];
