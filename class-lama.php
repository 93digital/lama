...
class Lama {
		...
		if ( ( method_exists( $posts, 'have_posts' ) ) ) {
			if ( $posts->have_posts() ) {
				...
				while ( $posts->have_posts() ) {
					...
					do_action( 'lama_inside_loop__' . $name, $count, get_the_ID(), $params );
	...
	public static function add_search_filter( $args ) {
		...
		if ( isset( $args['icon'] ) ) {
			printf(
				'<button type="button" class="%s__icon" aria-label="Search Resource">%s</button>',
				esc_html( $args['class'] ),
				$args['icon']
			);
		}
		...
		if ( isset( $args['hide_empty'] ) ) {
			if ( is_object( $args['hide_empty'] ) ) {
				...
				$all_posts = get_posts(
					[
						'fields'         => 'ids',
						'post_type'      => self::$current_query->get('post_type'),
						'posts_per_page' => -1,
					 ]
				);
			}
	...
	public static function add_dropdown_filter( $args = [] ) {
		...
		printf(
			'<select name="%s-%s" id="filter-%s" data-filter="filter-%s" class="%s" %s>',
			esc_attr( $filter ),
			esc_attr( $name ),
			esc_attr( $name ),
			esc_attr( $name ),
		...
		if ( ! empty( $args['placeholder'] ) ) {
			printf(
				'<option %s disabled="disabled">%s</option>',
		 ...
		 */
		if ( $args['clearable'] ) {
			$label          = is_string( $args['clearable'] ) ? $args['clearable'] : __( 'All' );
			if ( $args['custom-name'] ) {
	...
	public static function pagination( & $query, $show_ends = true, $params = [] ) {
		...
		$args = [
			...
			'total'     => $total_pages,
			'prev_text' => '<span class="pagination__arrow">' . $icon . '<span class="screen-reader-text">' . __( 'Previous Page', 'stella' ) . '</span></span>',
			'next_text' => '<span class="pagination__arrow">' . $icon . '<span class="screen-reader-text">' . __( 'Next Page', 'stella' ) . '</span></span>',
		];
